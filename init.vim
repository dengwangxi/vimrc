...
function! ExpandSnippetJumpOrTab()
    let snippet = UltiSnips#ExpandSnippetOrJump()
    if g:ulti_expand_or_jump_res > 0
        return snippet
    else
...
inoremap <expr> <Tab> pumvisible() ? "\<C-n>" :  "<C-R>=ExpandSnippetJumpOrTab()\<CR>"
...
if executable('ag')
 ...
 let g:ack_mappings = {
      \ "t": ":pclose<CR><C-W><CR><C-W>T:cclose<CR>",
      \ "T": ":pclose<CR><C-W><CR><C-W>TgT<C-W>j:cclose<CR>",
      \ "o": ":pclose<CR><CR>:cclose<CR>",
      \ "O": ":pclose<CR><CR><C-W>p<C-W>c:cclose<CR>",
      \ "go": ":pclose<CR><CR><C-W>p:cclose<CR>",
      \ "h": ":pclose<CR><C-W><CR><C-W>K:cclose<CR>",
      \ "H": ":pclose<CR><C-W><CR><C-W>K<C-W>b:cclose<CR>",
      \ "v": ":call DwxPreviewGoto()<CR>",
      \ "gv": ":pclose<CR><C-W><CR><C-W>H<C-W>b<C-W>J:cclose<CR>" }

      "\ "v": ":pclose<CR><C-W><CR><C-W>H<C-W>b<C-W>J<C-W>t:cclose<CR>",
...
nnoremap <leader>ag :call AgCurrWord()<CR>
nnoremap <leader>a :Ack<CR>
nnoremap <leader>as :AckFromSearch<CR>
nnoremap <leader>af :AckFile<CR>
...
let $GTAGSCONF = '/data/dev/.globalrc'
...
function! DwxPreviewGoto()
    let l:buffersBeforePreview = filter(range(1, bufnr('$')), 'buflisted(v:val)')
    let l:quickfixId = l:buffersBeforePreview[-1]
    execute ":PreviewGoto vsplit"
    let l:nowBuffers = filter(range(1, bufnr('$')), 'buflisted(v:val)')
    let l:lastId = l:nowBuffers[-1]
    "echom "dwx-".bufname(l:lastId)."-dwx"
    if bufname(l:lastId) == "" && l:lastId != l:quickfixId
...
autocmd FileType qf nnoremap <silent><buffer> q :call DwxQuickfixQ()<cr>
nnoremap q :call DwxQuickfixQ()<cr>
...
map <C-p> :PreviewTag<CR>
"nnoremap <leader>o :call DwxPreviewGoto()<CR>
